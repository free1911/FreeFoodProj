...
public class MAIN extends Application{
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        scrollBar.setMax(event.size()*25);
        scrollBar.setMin(0);
        scrollBar.setPrefHeight(350);
        scrollBar.setPrefWidth(10);
        scrollBar.setLayoutX(290);
        scrollBar.valueProperty().addListener(new ChangeListener<Number>() {
            @Override
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
                layout.setLayoutY(-newValue.doubleValue()*2);
            }
        });
    ...
    public void initialScene(int id){
        ...
        layout.getChildren().addAll(lb1,lb2,lb3,lb4,lb5,lb6,new mapPane(event.get(id).location),back);
